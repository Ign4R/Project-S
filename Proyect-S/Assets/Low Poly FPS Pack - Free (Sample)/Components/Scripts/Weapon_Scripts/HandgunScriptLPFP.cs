...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	public bool withoutKnife;
	private bool withoutGun;
	...
	private void Update () {
		...
		if (Input.GetKeyDown (KeyCode.Q) && !isInspecting && !withoutKnife) 
		{
			anim.Play ("Knife Attack 1", 0, 0f);
		}
		//Play knife attack 2 animation when F key is pressed
		if (Input.GetKeyDown(KeyCode.F) && !isInspecting && !withoutKnife) 
		{
			anim.Play ("Knife Attack 2", 0, 0f);
		}
			
		//Throw grenade when pressing G key
		if (Input.GetKeyDown (KeyCode.G) && !isInspecting && !withoutGun) 
		{
		...
		//Shooting 
		if (Input.GetMouseButtonDown (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning && !holstered && !withoutGun) 
		{
			anim.Play ("Fire", 0, 0f);
		...
		//Holster anim toggle
		if (holstered == true || withoutGun == true)  
		{
			anim.SetBool ("Holster", true);
		} 
		else if(!holstered || !withoutGun)
		{
			anim.SetBool ("Holster", false);
		}
		//Reload 
		if (Input.GetKeyDown (KeyCode.R) && !isReloading && !isInspecting && !withoutGun) 
		{
			//Reload
			Reload ();
